...
```
cd~/sysu-2026-spring-operating-system/lab3/src/example-2
...
；把反汇编输出格式切换成 Intel 风格语法。
；影响你看到的汇编展示方式，不改变程序执行结果。
b *0x7c00
c
```
...
| break symbol或b symbol           | 在符号symbol处设置断点，例如symbol一般是函数名。                                                                                                                                                                                                                                                                                                                                                                     | break setup_kernel`<br>`b setup_kernel |
...
| focus layout_window              | 转换当前窗口到layout窗口，layout_window包含src，asm，regs，cmd。任何时刻gdb的当前窗口只有一个，并且使用方向键的效果只会在当前窗口处显示。`<bt/>`                                                                                                                                                                                                                                                                     | focus cmd                              |
...
明白，你现在还是看到 `No source available`。这通常是下面两类问题之一：

1. 当前 `PC` 跑到没有符号的区域（比如 BIOS/QEMU ROM），不是你的代码。
2. 你的 `.symbol` 里没有行号调试信息（只有符号名，没有 source line）。

按这套最短流程排查，基本能定位：（这个有用）

1. 先重编译（强制带 DWARF 行号）

nasm-felf32-g-Fdwarf-ombr.ombr.asm

ld-melf_i386-N-Ttext0x7c00-ombr.symbolmbr.o

nasm-felf32-g-Fdwarf-obootloader.obootloader.asm

ld-melf_i386-N-Ttext0x7e00-obootloader.symbolbootloader.o

2. 检查 `.symbol` 里是否真有行号

- readelf --debug-dump=decodedline mbr.symbol | head
  readelf --debug-dump=decodedline bootloader.symbol | head

如果这里几乎空的，GDB 就不可能显示源码。
...
directory /home/zhengyy/sysu-2026-spring-operating-system/lab3/src/example-2
...
- 如果 `info line *$pc` 返回“没有行信息”，问题在编译/符号文件。
- 如果有行信息但 [layout src](vscode-file://vscode-app/c:/Users/YuanyuZheng/AppData/Local/Programs/Microsoft%20VS%20Code/ce099c1ed2/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 还是空，问题在源码路径（`directory` 没配对）。
